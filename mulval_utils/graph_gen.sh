...
if [ -f trace_output.P ]; then
    ...
    $MULVALROOT/bin/attack_graph $ATTACK_GRAPH_OPTS trace_output.P > "graphs/$INPUT-AttackGraph.txt"
	...
	grep -E "AND|OR|LEAF" AttackGraph.txt > "graphs/$INPUT-VERTICES.CSV"
	grep -Ev "AND|OR|LEAF" AttackGraph.txt > "graphs/$INPUT-ARCS.CSV"
